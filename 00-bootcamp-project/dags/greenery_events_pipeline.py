...
def _load_data_to_gcs(ds):
    keyfile_gcs = f"{DAGS_FOLDER}/deb-200024-load-data-to-gcs.json"
...
def _load_data_from_gcs_to_bigquery(ds, ds_nodash):
    keyfile_bigquery = f"{DAGS_FOLDER}/deb-200024-load-data-to-gcs-then-bq.json"
    ...
    table_id = f"{PROJECT_ID}.my_deb_bootcamp.{DATA}${ds_nodash}"
...
with DAG(
    dag_id="greenery_events_pipeline",
